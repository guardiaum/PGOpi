...
def my_args():
    ...
    if with_other==True:
        ...
        model_name = "BLSTM+MLP_with-other"
    else:
...
def model(checkpoint_path, embed_dim, n_classes, my_training_batch_generator, my_validation_batch_generator, train_steps, valid_steps):
    ...
    concat_ = Concatenate()([sentence_, aspect_])
    
    out_ = Dense({{choice([100, 200, 400, 600, 800])}}, activation='relu', name='dense')(concat_)
...
y_prob = best_model.predict([val_sentences, val_aspect])
...
y_prob_test_bb = best_model.predict([bb_test_sentences, bb_test_aspect])
...
y_prob_test_amzn = best_model.predict([amzn_test_sentences, amzn_test_aspect])
