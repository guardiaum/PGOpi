...
def my_args():
    ...
    if with_other==True:
        ...
        model_name = "BLSTM+MLP-sentence_with-other"
    else:
        ...
        model_name = "BLSTM+MLP-sentence_no-other"
...
def data():
    class My_Custom_Generator(tf.keras.utils.Sequence):
        def __init__(self, sentence, labels, batch_size):
        ...
        def __getitem__(self, idx):
            batch_sent = self.sentence[idx * batch_size : (idx+1) * batch_size]
            batch_y = self.labels[idx * batch_size : (idx+1) * batch_size]

            return ({'sentence_input': batch_sent}, {'out': batch_y})
    ...
    my_training_batch_generator = My_Custom_Generator(train_sentences, 
                                                      train_labels, batch_size)
    my_validation_batch_generator = My_Custom_Generator(val_sentences, 
...
y_prob = best_model.predict([val_sentences])
...
bb_test_sentences = load(test_path + 'X_test_bb_sentences.npy')
...
y_prob_test_bb = best_model.predict([bb_test_sentences])
...
amzn_test_sentences = load(test_path + 'X_test_amzn_sentences.npy')
...
y_prob_test_amzn = best_model.predict([amzn_test_sentences])
